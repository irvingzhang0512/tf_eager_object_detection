...
class RPNTrainingProposal(tf.keras.Model):
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        print(anchors.shape, gt_bboxes.shape)
...
class RPNProposal(tf.keras.Model):
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        # 3. 对选中修正后的anchors进行处理
        decoded_bboxes, selected_idx = _proposal_filter(decoded_bboxes,
...
def _proposal_filter(rpn_proposals, min_value, max_height, max_width, min_edge):
    """
    numpy 操作
