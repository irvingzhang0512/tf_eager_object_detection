...
def generate_by_anchor_base_tf(anchor_base, feat_stride, height, width):
    ...
    shift_y = tf.range(0, height * feat_stride, feat_stride, dtype=tf.float32)
    shift_x = tf.range(0, width * feat_stride, feat_stride, dtype=tf.float32)
    ...
    A = anchor_base.shape[0]
    K = shift.shape[0]
    anchor = tf.reshape(anchor_base, [1, A, 4]) + tf.transpose(tf.reshape(shift, [1, K, 4]), [1, 0, 2])
